...
(function() {
  ...
  function createLinkHotspotElement(hotspot) {
    ...
    icon.src = 'static/images/img/link.png';
  ...
  function createInfoHotspotElement(hotspot) {
    ...
    var icon = document.createElement('img');
    icon.src = 'static/images/img/info.jpg';
...
// If this hotspot is marked as "menu", open the shared menu overlay instead of toggle
if (hotspot.title=="Menu") {
  wrapper.querySelector('.info-hotspot-header').addEventListener('click', function () {
    openMenu();  // uses the helper we wrote earlier
  });
} else {
  // Default behavior
...
// ========== Custom HTML-based external hotspot handling ==========
function attachHtmlHotspots(sceneId, sceneObj) {
  // Hide all custom hotspots first
  document.querySelectorAll('.external-hotspot').forEach(el => {
    el.style.display = 'none';
  });

  // Show only hotspots belonging to this scene
  const sceneHotspots = document.querySelectorAll(`.external-hotspot[data-scene="${sceneId}"]`);
  sceneHotspots.forEach(el => {
    el.style.display = 'block';
    const yaw = parseFloat(el.getAttribute('data-yaw'));
    const pitch = parseFloat(el.getAttribute('data-pitch'));
    sceneObj.hotspotContainer().createHotspot(el, { yaw, pitch });
  });
}

// Modify switchScene to call attachHtmlHotspots
...
switchScene = function(scene) {
  ...
  // Attach our HTML hotspot for this scene
